...
An example `.fastq` file has been provided for you in the folder `/home/participant/Course_Materials/data/test`. If you are curious how this fastq file was generated, you can see the [Appendix](#appendix) for details.
...
```{}
cd /home/participant/Course_Materials/data/test
```
...
***Important*** The example files described were provided for the CRUK Bioinformatics Summer School; Analysis of Cancer Genomes - July 2017. If you do not see any `.fastq` files listed in the directory, you will need to download them. A script `download.sh` is provided for this purpose:-
...
```{r eval=FALSE}
library(ShortRead)
fq <- readFastq("/home/participant/Course_Materials/data/test/sample.fq1")
fq
```
...
The `html` file can be opened in a web browser. If you follow the link on the Desktop to `Course_Materials` you should be able to navigate to the folder `data/test/` and see the files we have created.
